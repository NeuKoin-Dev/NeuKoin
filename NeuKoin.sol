...
* Whitepaper: 	https://neukoin.com/NeuKoin-Creations-Whitepaper.pdf
...
contract NEUK is Context, IERC20, Ownable {
    ...
    string private constant _name = "NeuKoin Creations";
    string private constant _symbol = "NEUK";
    uint8 private constant _decimals = 9;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 3 * 10**9 * 10**_decimals;
    ...
    uint256 public numTokensSellToAddToLiquidity = 3 * 10**6 * 10**_decimals;   //0.1%
    ...
    */
    function setRfiRatesPercents(uint8 _rfi, uint8 _lp, uint8 _marketing, uint8 _dev) public onlyOwner {
      feeRates.rfi = _rfi;
    ...
    function setWallets(address _marketing, address _dev) public onlyOwner {
   ...
   function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount.sub(s.tRfi).sub(s.tLiquidity).sub(s.tMarketing).sub(s.tDev);
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi) {
        ...
        rTransferAmount = rAmount.sub(rRfi).sub(rLiquidity).sub(rMarketing).sub(rDev);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        reflectDevandMarketingFee(s.tDev,s.tMarketing);

        emit Transfer(sender, recipient, s.tTransferAmount);
    }


    function reflectDevandMarketingFee(uint256 tDev, uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
